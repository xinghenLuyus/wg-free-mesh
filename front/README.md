...
- 工具列表提供“下载”入口，进入后通过两个大卡片分别进入客户端下载和配置批量下载
- 客户端下载和配置批量下载子页面提供返回下载入口
- MQTT 服务关闭时保留相关卡片、按钮和表单布局并统一置灰；端点控制和客户端下载专属页面禁止进入
- 客户端下载页只提供一个下载动作；GitHub Release 源直接跳转 Release 下载，本地构建源由后端生成产物，前端不展示后端缓存状态
- 工具列表同时提供“快速组网”入口，进入后通过三个大卡片分别进入网关节点式网络、全连接网络和 Free Mesh；Free Mesh 卡片单独换行展示
- 快速组网子页面必须明示会删除该配置下所有 Mesh 对，可选择是否启用 PSK，生成期间用遮罩阻止交互
