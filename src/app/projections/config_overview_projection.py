...
from app.core.features import effective_node, effective_node_type
...
class ConfigOverviewProjection:
    def project(
        ...
        dynamic_node_ids = {node.id for node in active_nodes if effective_node_type(node) == NodeType.dynamic}
        node_card = lambda node: {
            ...
            "online": node.enabled
            and effective_node_type(node) == NodeType.dynamic
        ...
        return {
            ...
            "stats": {
                ...
                "dynamic_nodes": len([node for node in active_nodes if effective_node_type(node) == NodeType.dynamic]),
                "static_nodes": len([node for node in active_nodes if effective_node_type(node) == NodeType.static]),
            ...
            "nodes": [effective_node(node) for node in nodes],
