...
class SystemStatusProjection:
    def project(
        ...
        enabled_dynamic_node_ids = {
            node.id for node in nodes if node.config_id in enabled_config_ids and node.enabled and effective_node_type(node) == NodeType.dynamic
        }
        ...
        return {
            "summary": {
                "configs": len(configs),
                "nodes": len(nodes),
                "dynamic_nodes": len([node for node in nodes if node.enabled and effective_node_type(node) == NodeType.dynamic]),
                "online_nodes": len(
