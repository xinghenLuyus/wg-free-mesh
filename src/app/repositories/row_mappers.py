...
def config_from_row(row: Row) -> Config:
    return Config(
        ...
        online_node_count=int(row["online_node_count"] or 0) if "online_node_count" in row.keys() else 0,
        offline_node_count=int(row["offline_node_count"] or 0) if "offline_node_count" in row.keys() else 0,
        disabled_node_count=int(row["disabled_node_count"] or 0) if "disabled_node_count" in row.keys() else 0,
        pending_sync_node_count=int(row["pending_sync_node_count"] or 0) if "pending_sync_node_count" in row.keys() else 0,
        latest_config_node_count=int(row["latest_config_node_count"] or 0) if "latest_config_node_count" in row.keys() else 0,
        topology_invalid=bool_value(row["topology_invalid"]) if "topology_invalid" in row.keys() else False,
        topology_error_count=int(row["topology_error_count"] or 0) if "topology_error_count" in row.keys() else 0,
