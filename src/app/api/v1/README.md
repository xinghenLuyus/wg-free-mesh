...
- 修改路径、请求体、响应体或错误码时，同步更新 [API 契约](../../../../docs/developer/api-contract.md)、[后端设计](../../../../docs/developer/backend.md) 和相关参考文档。
- `settings.py` 同时负责控制台界面偏好、MQTT 设置和密码修改，不把轻量偏好拆成独立业务域。
- MQTT 设置只包含客户端对外可见的 Host / Port / TLS；整条客户端 MQTT 能力的启停只由部署环境变量 `WFM_ENABLE_MQTT_SERVICES` 控制，关闭后相关 bind 与远程控制接口必须返回禁用状态。
