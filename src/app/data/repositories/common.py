...
from app.domain.models import ControlAction, NodeType, TunnelProtocol
...
def link_payload(payload: dict[str, object], direction: str, row: Any | None = None) -> dict[str, object]:
...
def tunnel_protocol_value(value: object) -> TunnelProtocol:
    if isinstance(value, TunnelProtocol):
        return value
    raw = str(value or TunnelProtocol.wireguard.value)
    try:
        return TunnelProtocol(raw)
    except ValueError as exc:
        raise AppError("INVALID_TUNNEL_PROTOCOL", "Tunnel protocol is invalid", 400, {"value": raw}) from exc


def payload_string_list(value: object, default: Iterable[str] = ()) -> list[str]:
    if value is None:
