...
def test_create_config_node_and_mesh_flow() -> None:
    with TestClient(app) as client:
        ...
        sync_status = client.get(f"/api/v1/configs/{config['id']}/nodes/{node['id']}/sync-status")
        assert sync_status.status_code == 200
        assert sync_status.json()["data"]["node_id"] == node["id"]


def test_unknown_config_returns_structured_error() -> None:
    with TestClient(app) as client:
