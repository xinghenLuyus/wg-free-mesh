...
本文定义当前后台管理端采用的 SSE 实时推送架构。

重构目标：

- 浏览器后台统一使用 SSE 作为实时推送通道。
...
- 当前需求并不需要复杂的浏览器双向会话控制，复杂度与收益不匹配。
...
浏览器后台统一使用 `/api/v1/events/stream` 作为主实时入口。

后续如果确实存在特殊双向控制需求，再单独为客户端 Agent 或特殊控制场景设计独立通道，不与后台页面共用。
...
首版不单独设计纯 heartbeat 事件，而是让系统时间同步事件兼作活性信号。

建议做法：

1. 连接建立时立即发送一次服务端时间。
2. 连接存活期间每 `10s` 到 `15s` 再发送一次时间同步事件。
3. 前端本地按秒推进时间显示。
4. 下一次收到服务端时间后再校准。

推荐首版：

- `15s` 一次 `system.clock.sync`
...
如果后续部署环境对空闲连接更敏感，再追加注释型 keepalive：
...
但默认方案优先使用 `system.clock.sync` 兼作轻量保活信号。
...
旧的逐秒时间推送虽然简单，但会制造高频事件。

SSE 重构后建议改为：

1. 建立连接时后端返回一次服务端时间基线。
2. 前端在本地按秒推进显示。
3. 后端每 `10s` 到 `15s` 推送一次校时事件，并兼作连接活性信号。
...
- 前端没有打开网页时，不会建立 SSE 连接。
- 没有连接时，服务端不应持续向外推送事件。
- 页面关闭、刷新、登出或网络断开后，服务端必须尽快回收订阅队列。

因此 SSE 的流量消耗主要由两部分决定：
...
后端继续保留统一实时发布服务，输出通道统一为 SSE 可消费的事件流。
...
如果后续代理环境仍会过早切断空闲连接，再额外补充注释型 keepalive。
...
- 代理超时要放宽
- Docker 部署不需要再为浏览器后台承担额外的双向连接兼容处理
...
- 管理后台完全切到 SSE
- 删除浏览器后台对旧实时通道的依赖
- 是否保留其它专用通道，取决于后续是否还有 Agent 或特殊双向控制需求

## 本次重构建议的明确取舍

为了尽快稳定系统，建议首版采用以下取舍：

- 使用 SSE，浏览器后台只保留单一实时通道。
- 使用 Bearer Token Header，不走 URL token。
- 使用单一全局 SSE 流，不按页面拆多条连接。
- 不实现事件重放，断线后重新拉当前页面快照。
- 系统时间改为“后端低频校时 + 前端本地走秒”，由 `system.clock.sync` 兼作活性信号。
