...
- 客户端绑定不可用。
- 端点控制不可用。
- 禁止创建动态端点；已有动态端点在业务上按静态端点处理，数据库原始数据保持不变。
- MQTT 相关写接口返回 `MQTT_DISABLED`，配置同步止于服务端 staged 状态，不再下发。
- 前端保留 MQTT 相关布局，但控件置灰，控制和客户端下载页面不可进入。
- 后端不应依赖 EMQX 启动。

`COMPOSE_PROFILES` 独立控制 Docker 是否部署 EMQX。启用 MQTT 时，后端会在 EMQX 在线后同步账号、授权和客户端状态。
