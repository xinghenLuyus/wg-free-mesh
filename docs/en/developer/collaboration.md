...
- Normal pushes do not publish images or create Releases.
- Only pushed `v*` Git tags trigger image publishing and GitHub Release creation.
...
- The Git tag must be `v<version>`. Mismatches fail the release.
- Docker compose pulls `latest` by default. Production deployments can pin a version with `WFM_IMAGE_TAG` in `.env`.
- GitHub Releases automatically upload the multi-platform client zip packages generated by `client/build_release.py`.
- After a Release is created, its title, notes, prerelease flag, and assets can still be edited manually in GitHub.
